# Spreadsheets — source data inventory
...
spreadsheets themselves aren't in this repo yet — most have informal internal
team notes (cell comments, working columns) naming individual reviewers, which
haven't been reviewed for public posting. This documents what's in each one in
the meantime.
...
Not uploaded here yet either, for the same reason as the file above - its Notes column
directly quotes the internal cell comments and reviewer notes from
`Data_Project_brain.cell.killer.xlsx`, so it carries the same names/comments that haven't
been reviewed for public posting. (It's also being generated in the separate LPI-Arecibo-data
repo at the time of writing - check there if you need it before it lands here.)
