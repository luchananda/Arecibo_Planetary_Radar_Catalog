...
d = json.load(open(f'{PROJ}/FINAL_TABLE_all.json'))
try:
...
<title>Planetary Radar All-Attempts Catalog</title>
<style>
.ao-dash {{
...
.all-badge {{ display: inline-block; vertical-align: middle; font-size: 11px; font-weight: 700; letter-spacing: .03em; background: var(--gold); color: #111; padding: 3px 9px; border-radius: 999px; margin-left: 8px; }}
.uc-banner {{ display: block; text-align: center; font-size: 15px; font-weight: 800; letter-spacing: .06em; text-transform: uppercase; color: #fff; background: #b91c1c; padding: 10px 14px; border-radius: 8px; margin: 0 0 14px 0; animation: uc-flash 1.1s step-start infinite; }}
@keyframes uc-flash {{ 50% {{ opacity: .35; }} }}
@media (prefers-reduced-motion: reduce) {{ .uc-banner {{ animation: none; }} }}
...
.notes-compact {{ border-bottom: 1px dotted var(--muted); cursor: help; color: var(--text-secondary); font-size: 11.5px; }}
...
.ao-modal-field .fv {{ font-size: 12.8px; font-variant-numeric: tabular-nums; margin-top: 1px; word-break: break-word; }}
      ...
      <table class="ao-table" id="ao-table">
        <thead>
          <tr>
            ...
            <th data-k="h">H (mag) <span class="fl-help" tabindex="0" data-tip="Value for non-detections obtained from SBDB.">i</span></th>
...
(function() {{
  ...
  function render() {{
    ...
    var html = pageRows.map(function(r) {{
      ...
      var detCell = r.det === 'Yes' ? '<span class="badge badge-yes">Detected</span>'
                  ...
                  : (r.det === 'Unclear' ? '<span class="badge badge-no">Unclear</span>'
                  : '<span class="badge badge-no">&mdash;</span>'));
      var notesCell = r.notes ? '<span class="notes-compact" title="' + esc(r.notes) + '">' + esc(r.notes.length > 60 ? r.notes.slice(0, 60) + '\\u2026' : r.notes) + '</span>' : '<span class="ref-none">&mdash;</span>';
      return '<tr' + trAttrs + '><td>' + esc(r.num) + '</td><td>' + nameCell + '</td><td>' + desigCell + '</td><td>' + esc(r.cat) +
        ...
        '</td><td class="lpi-col">' + lpiCell + '</td><td>' + refsCell + '</td><td>' + notesCell + '</td></tr>';
    }}).join('');
    tb.innerHTML = html;
  ...
  var CARD_FIELD_ORDER = ['Category', 'Bodies', 'Absolute Magnitude (H)', 'Quality Code',
                          'Years Observed', 'Diameter (km)', 'Rotation Period (h)', 'Geometric Albedo', 'Notes'];
  ...
  function buildFieldSources(row, info) {{
    ...
    var notesSrc = 'ALL_AO_CATALOG_2026_DRAFT.xlsx, Verified_Catalog sheet (Comment - NOTE column, or Why not on webpage/How confirmed when that was blank)';
    ...
    CARD_FIELD_ORDER.forEach(function(k) {{
      ...
      else if (k === 'Notes') out[k] = notesSrc;
  ...
  document.getElementById('ao-table').querySelector('thead').addEventListener('click', function(e) {{
    if (e.target.closest && e.target.closest('.fl-help')) return;
...
open(f'{PROJ}/dashboard_all.html', 'w').write(page)
print(f'Saved dashboard_all.html: {len(page):,} bytes, {len(rows)} rows embedded')
