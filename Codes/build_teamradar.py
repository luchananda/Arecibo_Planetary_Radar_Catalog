#!/usr/bin/env python3
"""Build TeamRadar-Revision.html (or TeamRadar-Revision-all.html): a PREVIEW
of the interactive team-review page, built by post-processing the
already-built dashboard.html/dashboard_all.html (reuses its object
data/table/grid/modal wholesale, adds a team-input layer on top).

Pass 'all' as the first argument to build the all-attempts-catalog variant
(TeamRadar-Revision-all.html, wrapping dashboard_all.html) instead of the
default detected-only one. Both currently submit to the SAME shared Google
Form/Sheet - if that turns out to be confusing once real review starts on
the all-attempts side, a second Form is needed (Luisa has to create it; the
entry IDs would need to be swapped in below)."""

import re
import sys

PROJ = '/Users/floridaspaceinstitute/Documents/Radar/Catalogue4Webpage'
MODE = sys.argv[1] if len(sys.argv) > 1 else 'detected'
assert MODE in ('detected', 'all'), "MODE must be 'detected' or 'all'"

if MODE == 'all':
    SRC_FILE = 'dashboard_all.html'
    OUT_FILE = 'TeamRadar-Revision-all.html'
    PAGE_TITLE = 'TeamRadar Review — All-Attempts Catalog'
    ...
    H1_REPLACEMENT = (
        ...
        'catalog automatically &mdash; a person reviews the sheet and applies accepted changes to the main catalog by hand.</div>')
else:
    SRC_FILE = 'dashboard.html'
    OUT_FILE = 'TeamRadar-Revision.html'
    PAGE_TITLE = 'TeamRadar Review — Detected Objects Catalog'
    H1_ANCHOR = '<h1>Arecibo Observatory &mdash; Planetary Radar Detected Object Catalog</h1>'
    H1_REPLACEMENT = (
        '<h1>TeamRadar Review &mdash; Detected Objects Catalog</h1>\n'
        '<div class="tr-banner">TEAM REVIEW BUILD &mdash; reviews the <b>detected-objects</b> catalog only (not the '
        'all-attempts catalog), separate from the live dashboard above. Submissions here ARE saved live to a shared '
        'Google Sheet (ask apophis@ucf.edu for access). They do not update the catalog automatically &mdash; a person '
        'reviews the sheet and applies accepted changes to the main catalog by hand.</div>')

html = open(f'{PROJ}/{SRC_FILE}', encoding='utf-8').read()

# 1) title + banner
if re.search(r'<title>.*?</title>', html):
    html = re.sub(r'<title>.*?</title>', '<title>' + PAGE_TITLE + '</title>', html, count=1)
else:
    html = html.replace(
        '<meta charset="utf-8">',
        '<meta charset="utf-8">\n<title>' + PAGE_TITLE + '</title>',
        1)
assert html.count(H1_ANCHOR) == 1, f'h1 anchor not found exactly once in {SRC_FILE} - update it'
html = html.replace(H1_ANCHOR, H1_REPLACEMENT, 1)
...
open(f'{PROJ}/{OUT_FILE}', 'w', encoding='utf-8').write(html)
